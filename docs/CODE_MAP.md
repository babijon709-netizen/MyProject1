...
| флаг «программа работает», шаг кадра и его пульс, остановка потоков | `jni/src/app/lifecycle.cpp` |
...
sh tools/diag/run.sh           # журнал здоровья: запись не тормозит кадр
sh tools/memio/run.sh          # доступ к памяти игры (чтение через кэш)
sh tools/touch/run.sh          # инъекция тача (снятие пальцев, повторы)
...
sh tools/espmath/run.sh        # мировой->экранной математики (настоящий esp/math.cpp)
```

Стенд `tools/espmath` появился 17 сентября 2026 после ошибки знака в переводе
матрицы вида (`mat_view_from_basis`): числа ломались только при камере вдали от
начала мира, а сборка и логика при этом проходили все остальные проверки. Стенд
собирает настоящий `jni/src/esp/math.cpp` и проверяет числа: точку прямо перед
камерой, «выше/правее», точку за камерой, разбор кэша проекции и совпадение вида
из кватерниона с видом из базиса.
...
поехал дальше. Порядок проверки: углы прицела самой игры (MouseLook+0x4C, взяты
...
замечается за три чтения указателей, а не записью в освобождённую память. Углы MouseLook (0x4C/0x28)
одинаковы в релизе и бете; пара углов оружия в бете сдвинута на 8 байт (0x2DC
против релизных 0x2D4) — проверено дизассемблером beta libil2cpp (Oxide.FPManager
$$swg, RVA 0x657cce4, пишет ту же пару тем же кодом), поэтому смещение берётся из
переключателя версий (`FPHITSAN_LOOK_ANGLES_OFFSET` в `game_offsets*.h`).
...
«Мемори» молчал «не находит поворот камеры» (xvcen_health.log). Замер пробного
доворота берётся у независимого свидетеля — камеры или узла прицела
(`read_measured_angles`), — а если свидетеля нет вовсе, по самому полю: оно
состояние игры, и затирание записи видно по нему же.

Самотест ограничен по времени (6 с): не нашлось — режим «Мемори» объявляет отказ
с причиной, и аим продолжает работать тач-веткой.
...
Что тут важно: пока с момента привязки не опубликовано НИ ОДНОГО кадра, сторож
не сбрасывает кэши и не просит перепривязку — игра может просто ещё не отдать мир
(«кадр не собран: нет ссылки на камеру игры»). Состав игроков держится и тогда,
когда список вообще не читается: раньше через 5 с кэши мира обнулялись, и каждый
такой сброс выглядел как «ESP моргнул, а потом боксы появлялись не сразу» —
теперь порог 12 с, а до него кадр помечается устаревшим
(`g_player_data_stale`), и рисующий слой держит рамки по длинному окну. Полная
смена состава тоже требует больше кадров подряд (5 вместо 3): адреса списка
иногда мигают, и одиночный мусорный кадр не должен стирать кэши.
Ещё в журнал идёт отчёт «рамок нет: …» (не чаще раза в 5 с) с разбором причин —
в списке, скрыто, без позиции, далеко, ниже/выше экрана, низких, — и с флагами
«матрицы подтверждены» и «режим узла»: по логу устройства пустой кадр выглядит
одинаково во всех этих случаях, и без отчёта правка делается вслепую.
Раньше чит в этом состоянии каждые
3 с обнулял кэши мира, а после третьего раза уходил на перепривязку: терялись
подтверждённая раскладка, позиция игрока и самотест мемори-аима, и чит «оживал»
только через минуту. Теперь причина каждого несобранного кадра пишется в журнал
(не чаще раза в 5 с), а сам кадр собирается и без подтверждения раскладки: та
проверка («матрицы физичны») — уточнение о выборе локального игрока, а не условие
жизни кадра.
...
`reset_world_caches` чистит только то, что привязано к адресам объектов
(состав игроков, кэши игроков, раскладку скелета), но НЕ забывает подтверждённое
смещение позиции игрока: это свойство сборки игры, а не мира. Пока сброс его
забывал, первый кадр после смерти уходил искать смещение заново, ждал 0.6 с
«пока поля допишутся» и уходил на обход иерархии — то есть боксов после
перезагрузки мира не было вовсе.
...
секунд дёргал бы прицел пробными доворотами. Зато ПЕРВЫЙ подбор дорожки паузы не
знает: при включении «Мемори» он начинается сразу (`esp_mem_aim_power_on`), а
если сорвался на ещё не загруженном мире — повтор через 2 с, а не через 15 (это
и была жалоба «мемори начинает наводиться не сразу, а через время»). Сколько
секунд занял подбор, видно в журнале строкой «мемори-режим: дорожка записи
подтверждена за N с (путь M)». Ещё отсюда поднимается признак
...
Здесь же — сторож кадра (`FrameWatchTick`), ответ на жалобу «чит в один момент
полностью завис»: в журнале от такой поломки не оставалось ничего, потому что
встаёт ГЛАВНЫЙ поток (на хранилище, на обмене с системой, на снятии кадра), а
писать о поломке было некому. Главный поток теперь каждый кадр отмечает шаг и
время (`FrameStage`/`FrameDone` из `app/lifecycle.cpp`), а поток привязки раз в
1,5 с сверяет: нет кадра 6 с — пишет «КАДР НЕ ЗАВЕРШАЕТСЯ N с: шаг «…», поток
кадра в состоянии S/D/R» (состояние — из `/proc/self/task/<tid>/stat`), а после
20 с будит главный поток сигналом SIGUSR1. Обработчик SIGUSR1 в `main.cpp`
заведён БЕЗ `SA_RESTART`: с ним прерванный системный вызов перезапустился бы, и
зависший кадр сам бы не отошёл.
...
Что тут важно: запись АСИНХРОННАЯ. Журнал лежит на внешнем хранилище
(/storage/emulated/0/benzhack) — это FUSE, и его write() может встать на
секунды. Раньше строку писал тот, кто её позвал, под общим мьютексом: поток
кадра упирался в диск, остальные ждали мьютекс — ровно тот случай, когда чит «в
один момент полностью зависает» и записи в файле обрываются. Теперь `diag_log()`
только копирует строку в кольцевой буфер (64 КБ), а файл пишет отдельный поток
(`writer_thread`); переполнение не молчит — событие считается, и в файле
появляется строка «(потеряно событий: N)». `diag_flush` при выходе даёт хвосту
доехать. Стенд — `tools/diag/run.sh`.

Что тут важно ещё: это не отладочный вывод, а след для разбора живых жалоб («на
...
пришёл пустым, — до 1.5 с, а пока данные игроков не читаются
(g_player_data_stale) до 6 с (те же окна у трека позиций игрока в
`player_pose.cpp`, чтобы слои не разъезжались). Раньше пустой кадр отдавался как есть, и вместе с

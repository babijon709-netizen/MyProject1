...
**Определение типа — два источника.** Сначала `entityType`; у животных он
заполнен, у рудных узлов — нет (остаётся `None`). Поэтому вторым шагом читается
лут узла: `MineableObject.m_Loot` (0xA0, `List<Oxide.LootItem>`) →
`LootItem.ItemName` (0x10, строка shortname). Совпадения:
`sulfur*` → Сера, `metal*` → Железо, `ice*` → Лёд, `stone` → Камень;
`wood`/`cloth`/`raw.meat` не дают ничего, поэтому деревья, кусты и трупы
отсеиваются сами. Ранги нужны из-за того, что серные и железные узлы дают ещё и
камень: побеждает более ценный ресурс.
...
железо — оранжевый, сера — жёлтая, лёд — голубой. Цвет животных настраивается.
На экран выводятся только руды и животные, деревья и бочки отфильтрованы. Реестр пересканируется раз в ~3 с,

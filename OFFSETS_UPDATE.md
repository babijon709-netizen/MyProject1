...
6. Готовая подпись переводится на русский (`localize_weapon_label`,
   таблица `kWeaponNamesRu`). Ключи нормализуются (только буквы и цифры,
   в нижнем регистре), сравнение сначала точное, потом по самой длинной
   подстроке — поэтому `07_DefaultAssault Riffle`, `Assault Rifle` и
   `assault.rifle` дают один и тот же «Автомат», а `pickaxehammer`
   не путается с `pickaxe`. Незнакомое оружие показывается очищенным
   английским именем (не пропадает).

...
Подпись берётся из таблицы по словам имени (`military` → Военный ящик,
`airdrop`/`supply` → Аирдроп, `barrel` → Бочка, `crate`/`lootbox` → Ящик, …).
Ранг нужен, чтобы `MilitaryCrate` не превратился в простой «Ящик». Если ни одно
слово не совпало, но объект прошёл оба фильтра, он всё равно рисуется как
«Ящик» — лучше показать неизвестный мировой контейнер, чем потерять его.

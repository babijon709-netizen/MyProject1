...
* **Отчёт «рамок нет» ужат под 240 байт** (журнал режет строку, и прежняя
  длинная теряла как раз «матрицы подтверждены»): спис/свой/скрыт/безпоз/далеко/
  низ/верх/корот/матр/вид, затем позиция камеры, позиция игрока и его проекция с
  `w` — по `w <= 0` сразу видно «точка за камерой», по `экр y` — «ниже кадра».

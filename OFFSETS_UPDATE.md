...
* `*_TYPEINFO_RVA` — только если в архиве беты есть её `libil2cpp.so` (или он
  лежит рядом как `libil2cpp_beta.7z`); иначе останутся релизные, и бета-клиент
  не найдёт классы (скрипт предупреждает, а в меню видно строку-источник).

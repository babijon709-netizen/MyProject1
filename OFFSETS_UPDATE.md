...
### 17 сентября 2026: ESP — вид из углов прицела, проекция из кеша игры

Причина: в журнале устройства по каждому кадру без рамок стояло «ниже экрана N,
выше экрана 0» — то есть у ВСЕХ игроков не проходила даже нижняя точка тела.
`w2s` с `clip_to_screen = false` возвращает «нет» только когда точка ЗА камерой,
значит смотрел не туда не игрок, а наш вид: поза камеры с устройства читается не
всегда (в журнале аима это «cam_st = 0», см. также §3.1 — кеши 0x70/0xB0 ленивые).

Что сделано, и на чём это держится:

* **Вид из углов прицела игры.** `esp_read_look_angles` читает пару MouseLook
  (0x4C: x — рыскание, y — тангаж) — то же поле, куда пишет мемори-аим и которое
  на устройстве читается всегда (оно не зависит ни от Transform, ни от матриц).
  Если поза камеры расходится с этими углами больше чем на 60° три проверки
  подряд (~1 с), вид собирается из углов (`g_cam_view_source = 1`), позиция —
  по-прежнему из позы. Направление по углам берётся в общей конвенции проекта
  (`forward_from_look_angles` — обратная к `angles_from_forward`, которой меряет
  аим), поэтому вид согласован с тем, что аим пишет в прицел. Как только поза
  снова сходится (5 проверок), вид возвращается к ней.
* **Свидетель не из того же поля.** `read_witness_angles` в мемори-аиме больше не
  берёт камеру, когда она повёрнута ИЗ углов: это было бы чтение того же поля и
  «подтверждение» любой записи. Независима ось выстрела (LookDirection) и узел
  прицела — они и остаются свидетелями.
* **Проекция — из кеша самой игры** (0xB0, смещение проверено дизассемблером
  libunity.so): структура проверяется (`perspective_orientation`, ловит и
  построчную раскладку нативной `Matrix4x4f`), пропорция сверяется с экраном, и
  только если кеш негоден, считается своя перспектива по полю 0x170. Нужно это
  потому, что поле — угол по ОСИ камеры (`Camera.fovAxis`): если игра задаёт её
  по горизонтали, наша перспектива выходит слишком узкой и всё, что ниже центра
  кадра, уезжает за нижний край. Расхождение угла обзора пишется в журнал.
* **Ось прицела без камеры.** `read_local_aim_reference` больше не выходит, когда
  поза камеры не прочиталась (`g_aim_ref_unverified`): ось берётся из обработчика
  событий игрока, а глаз — из KCC. Раньше в этом случае аим молчал «поля поворота
  не найдены».
* **Отчёт «рамок нет» ужат под 240 байт** (журнал режет строку, и прежняя
  длинная теряла как раз «матрицы подтверждены»). Подписи сокращены до
  спис/свой/скр/бп/дл/низ/вх/кр/мт/вд, дальше позиция камеры (км), позиция
  первого отброшенного игрока (иг) и его проекция (эк, w). По `w <= 0` сразу
  видно «точка ЗА камерой», по `эк y` выше высоты экрана — «ниже кадра». Числа
  зажаты (±99999, w ±9999), иначе «улетевшее» чтение снова растянуло бы строку
  за предел и обрезка съела бы хвост с `w`.

...
| 2 | `crate`, `lootbox`, `loot`, `container`, `chest`, `case`, `cache`, `trash`, `sack` | Ящик / Контейнер / … |

Если ни одно слово не совпало, но объект прошёл оба фильтра, он всё равно
рисуется как «Ящик» — лучше показать неизвестный мировой контейнер, чем
потерять его.
...
`hq.metal` → Металл HQ, `metal.frag` → Фрагменты и ещё ~45 записей. Неизвестный
предмет не теряется: подпись берётся из `Item → ItemData.m_Name` (английское имя
из игры), а если и его нет — из самого шортнейма. При `amount > 1` к подписи

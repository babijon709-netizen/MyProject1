...
- **`jni/src/esp/`** — код чтения памяти (обход объектов, строки, боксы, ESP, камера, мемори-аим): по модулю на тему, карта «что править → файл» — `docs/CODE_MAP.md`.
- **`jni/src/mem_io.h`** — доступ к памяти игры: два способа чтения (process_vm_readv и /proc/<pid>/mem), выбор пробой и автопереключение, кэш блоков. Стенд — `tools/memio/run.sh`.
- **`jni/src/main.cpp` + `jni/src/ui/`, `jni/src/aim/`, `jni/src/farm/`** — окно меню, конфиги, аим и автофарм (оффсеты сами не читают, кроме констант через game_offsets).
- **`jni/Android.mk` / `build.sh`** — сборка `libs/arm64-v8a/xvcen.sh` (GitHub Actions собирает на push).

## 1.1. Как читается память игры (`jni/src/mem_io.h`)

Чтение и запись идут через один слой, и способ доступа там не один:

* `process_vm_readv` / `process_vm_writev` — быстрый путь, один syscall на пачку;
* `pread` / `pwrite` по `/proc/<pid>/mem` — второй путь, дескриптор открывается
  один раз и живёт до отвязки.

Способ выбирается пробой при привязке (читается ELF-заголовок по адресу начала
отображения `libil2cpp.so`) и автоматически меняется на второй, если выбранный
начал отказывать (8 отказов подряд). Причина не в стиле: политики прошивок режут
по-разному, и на части устройств запрещён именно `process_vm_readv` — привязка
«удавалась», меню рисовалось, а каждое чтение возвращало ошибку, то есть молча не
работала ни одна функция. Если не работает и второй способ, привязка честно
отказывает и в меню показывается причина («Нет доступа к памяти игры»).

Поверх — кэш блоков по 128 байт (8 блоков на поток): мелкие чтения полей внутри
кадра берутся из блока, а не из syscall'а. Кэш сбрасывается в начале кадра
(`esp_mem_frame_begin()` из главного цикла), а запись в память игры (Иксрей,
«Всегда день») помечает затронутый блок некэшируемым, поэтому своих же значений
из кэша не читаем. Замер стенда на профиле кадра ESP (40 объектов × 12 полей ×
8 кадров): 3840 syscall'ов → 320, время ×5.7.

Поиск процесса тоже другой: кандидат не «первый с совпавшим cmdline», а
проверенный по карте памяти (`libil2cpp.so` отображён); если такого нет — берётся
процесс-приложение (uid ≥ 10000) с отображённым `libil2cpp.so`, поэтому клон
приложения или сборка с другим applicationId тоже находятся. Пока привязка жива,
`/proc` не перебирается: раз в 1.5 с читается cmdline одного pid и одно чтение
памяти (`esp_alive_check()`), а перебор запускается только когда процесс ушёл или
доступ отобрали.

## 1.2. Мемори-аим: где игра держит поворот прицела

Режим «Мемори» во вкладке «Аим» доворачивает прицел записью в память игры
(`jni/src/esp/aim_mem.cpp`), поэтому ему надо знать, где лежит поворот. Три
константы живут не в `game_offsets.h`, а в `jni/src/esp/camera.h`: в релизе и
бете они совпадают, а `game_offsets.h` держит только расходящееся.

| что | где | значение |
|---|---|---|
| `PLAYER_MOUSE_LOOK_OFFSET` | `PlayerManager` → объект MouseLook | `0x70` |
| `MOUSE_LOOK_SENSITIVITY_OFFSET` | MouseLook → чувствительность `m_Sensitivity` | `0x34` |
| `MOUSE_LOOK_ACCUM_OFFSET` | MouseLook → накопленный сдвиг ввода (yaw; `+0x8C` — вторая ось) | `0x88` |

Откуда известно: в коде игры есть метод (RVA `0x64e312c` в обоих билдах), который
читает поле `+0x88`, умножает на `+0x34` и применяет как доворот камеры — это и
есть «палец на экране». Проверено на релизе `205619` и бете `207986`: раскладка
одна и та же, поэтому константы и не поехали в набор, который пересобирается
после каждого апдейта.

Важно: мемори-аим на эти числа НЕ полагается. Самотест пробует дорожки по
очереди (кватернион поворота, пара углов в градусах и радианах, поле
накопленного ввода, узел прицела в иерархии Transform) и принимает дорожку
только по замеру: пробный доворот yaw 2° / тангаж 1° обязан встать в допуске
0.6° и не поехать дальше. Если после апдейта игры что-то сдвинется, режим
объявит «через память нельзя» с причиной (строка состояния во вкладке «Аим»), а
аим продолжит работать тач-веткой — молча мазать он не станет.

Арифметика шага доворота проверяется стендом без устройства:
`sh tools/aim/run_mem.sh` (собирает настоящий `jni/src/aim/memory.cpp`).
...
6. **Смещения поворота прицела** (`PLAYER_MOUSE_LOOK_OFFSET` `0x70`,
   `MOUSE_LOOK_SENSITIVITY_OFFSET` `0x34`, `MOUSE_LOOK_ACCUM_OFFSET` `0x88` в
   `jni/src/esp/camera.h`) — в релизе и бете совпадают, поэтому в `game_offsets.h`
   их нет. Перепроверка после апдейта: разбор `PlayerManager`/MouseLook в новом
   `il2cpp.h` (см. §1.2) и, надёжнее всего, самотест на устройстве — вкладка
   «Аим», режим «Мемори», строка состояния («готово: …» или причина отказа).
...
### 17 сентября 2026: тач-аим — ошибка знака в переводе вида и подмена оси

Числа из журнала устройства (сессия со сборкой от 17.09.2026) в отчёте «рамок нет»
дали ответ сразу по двум жалобам:

* `эк -1 -1 w -1395.61` (и `w -5.91`, `-13.50`, `-35.73` в других кадрах) — у ВСЕХ
  игроков знаменатель проекции отрицательный, то есть в собранном виде они все ЗА
  камерой. `w2s` с `clip_to_screen = false` возвращает «нет» только в этом случае
  (или когда матрица нечитаемая), значит не игрок не туда стоит, а наш вид.
* `пара углов (вид 1) — 12.2, -171.7; углы прицела -171.4, -12.2` — игра хранит
  ту же пару углов, но РЫСКАНИЕ В СЛОТЕ 1, а тангаж в слоте 0 (и с обратным
  знаком). Перебор в подборе дорожки менял только знаки, порядок слотов был
  жёстко задан, поэтому такая пара не совпадала никогда: тангаж -171.7 сразу
  отбраковывался как выходящий за пределы, дорожка записи не находилась, и
  мемори-аим «как не был, так и не начал» работать.

Что сделано:

* **Ошибка знака в `mat_view_from_basis`.** Строка перевода была «упрощена» до
  `-(forward * position)` вместо `-(-forward * position)`: глубина кадра уезжала
  на `2 * (forward * position)`, то есть тем сильнее, чем дальше камера от начала
  мира. На этой матрице стоит ось аима (`esp_aim_camera_angles` считается по
  `g_cam_forward`), отсюда «целится в пустоту и отдергивает в сторону», когда цель
  входит в прицел. Ошибку ловит новый стенд `tools/espmath` (собирает настоящий
  `jni/src/esp/math.cpp` и проверяет числа проекции при камере вдали от начала
  мира); на сломанном знаке он валит 9 проверок из 38.
* **Вид больше не подменяется углами прицела.** `g_cam_view_source = 1` из сборки
  от 17.09 записывал в `g_cam_forward`/`g_cam_pose_valid` поворот ИЗ УГЛОВ, а по
  ним считается готовая ось аима. Углы и поза живут в разных конвенциях знаков,
  поэтому аим начинал вести в противоположную сторону. Остался только замер
  расхождения (`g_cam_view_angle_gap_deg`) — он идёт в журнал строкой «углы
  прицела и поза камеры расходятся на N°».
* **Проекция снова считается из поля угла обзора** (как до 17.09), а кеш 0xB0 —
  только последний резерв и замер расхождения. Причина: проекция стоит под
  наведением, и её подмена меняет экранные координаты всех целей; для фарма и аима
  это шаг назад. Если поле окажется углом по ОСИ (`Camera.fovAxis`), в журнале
  появится строка «угол обзора: поле N°, кеш камеры M°» — по ней и будем решать,
  а не подменой.
* **Ось прицела без камеры снова берётся** (`g_aim_ref_unverified`), как в сборке
  от 17.09: без позы камеры сверять не с чем, а ось — из обработчика событий
  игрока (LookDirection), и без неё аим молчит «поля поворота не найдены».
* **Порядок слотов пары вошёл в перебор** (`push_pair_candidate`): порядок × знак
  рыскания × знак тангажа = 8 комбинаций, и найденный порядок сохраняется в
  кандидате (куда писать рыскание, а куда тангаж — решает именно он). В журнал
  теперь идут слоты и знаки: «пара углов (вид N, слоты 1/0, знаки 1/-1)».

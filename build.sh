...
if command -v python3 >/dev/null 2>&1; then
    ...
    # Источник углов аима: аим не должен учить чувствительность по базису из
    # матрицы вида (он отстаёт на кадр — лог 15.09.2026: gain со сменой знака и
    # палец в край экрана). Проверка сторожит разделение источников по коду.
    sh "${ROOT}/tools/aim/check.sh"
else
    echo "python3 не найден — проверки переводов и источника углов аима пропущены" >&2
fi

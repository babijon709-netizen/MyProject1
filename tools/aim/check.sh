...
# 4. esp_aim_camera_angles не должен знать про базис из матрицы вида.
...
# 5. Базис по-прежнему доступен фарму — иначе он снова повиснет в «нет позиции
...
# 6. cam_st в логе обязан отражать источники аима: «cam_st 0 = настоящей оси нет»
...
print('аим: запасной коэффициент %.3f град/px (измерен и совпадает с фармом), '
      ...
      '(cam_st), базис из матрицы вида остаётся фарму' % field.get('probeGainYaw', 0.0))
PY

...
Русские строки — источник перевода: строка из XS("...") в модулях меню и подписи
визуалов из esp/*.cpp. Здесь же лежат их английские варианты. Подписи оружия не
пишутся руками: они берутся из таблицы самой игры (kWeaponNames в esp/weapons.cpp,
...
Скрипт заодно проверяет полноту: если в модуле появилась строка с кириллицей
...
#
#   MAIN — меню и всё остальное (jni/src/main.cpp, ui/*, app/*, lang.cpp);
...
UI_EN = {
    ...
    # режим аима (вкладка «Аим»): тач-палец или доворот записью в память
    'Режим': 'Mode', 'Тач': 'Touch', 'Мемори': 'Memory',
    ...
    'углы поворота, градусы': 'rotation angles, degrees',
    'углы поворота, радианы': 'rotation angles, radians',
    'ввод поворота игры': 'game look input',
    'узел камеры': 'camera transform',
    'нет доступа к MouseLook': 'no access to MouseLook',
    'углы прицела не читаются': 'camera angles are unreadable',
    'поля поворота не найдены': 'no rotation fields found',
    'запись не доворачивает прицел': 'the write does not turn the camera',

...
// Записать ввод взгляда в единицах игры: yaw_units — рысканье (+ вправо),
// pitch_units — вертикаль в том же знаке, в каком его ждёт игра (то есть уже
// с учётом m_Invert: см. реализацию в aim.cpp). false — некуда писать.
bool esp_mem_aim_write_look(float yaw_units, float pitch_units);
// Текущие углы взгляда из памяти игры (градусы): то, во что игра превратила
// наш ввод. Нужно, чтобы отличить «записали, но игра не отработала» от
// «записали и камера поехала». false — углы в этом кадре не читаются.
...
// Держать ось выстрела фоновым писателем (период ~2 мс, как у «всегда день»).
// Игра кладёт свою ось каждый кадр, поэтому одна запись за кадр оверлея
// оказывается перезаписанной до выстрела в половине кадров: писатель-доминатор
// добивает ось тем же направлением между кадрами. Пока включён, направление
// обновляется каждым вызовом esp_mem_aim_write_fire_dir().

...
struct EspBox {
    ...
    bool  aim_valid[3];
    // Мировая точка того же прицела (с упреждением по скорости и подъёмом
    // головы) — для аима без MouseLook, который вращает камеру прямо к
    // мировой точке, а не пишет углы в накопитель.
    float aim_world[3][3];
...
//
// Мемори-аим БЕЗ MouseLook (переделка 19.09: объект MouseLook терялся —
// адрес локального игрока ездил в кадре, объект умирал на респавне, класс
// учится один раз и потом служит пропуском — и каждый такой сбой молчал
// аимом). MouseLook и всё, что от него зависит (накопитель углов,
// чувствительность, владелец), больше не используется: камера вращается
// ПРЯМО, записью локального поворота её собственного Transform.
//
// Как. Каждый вызов:
//   1) слот трансформа камеры решается с нуля по ОБЪЕКТУ камеры
//      (Camera -> GameObject -> Transform -> его «данные/индекс» в массивах
//      иерархии) — не по кешу, не по перебору кандидатов;
//   2) позиция этого слота сверяется с позицией камеры из матрицы вида
//      (того самого, чем игра рисует кадр): разошлись на 2.5+ м — это не
//      слот камеры (пустая/переразделённая ячейка), отказ;
//   3) текущее направление камеры и направление на цель дают ось и угол
//      ошибки; записывается доворот на min(остаток, max_step_deg) вокруг
//      этой оси — в ЛОКАЛЬНЫЙ поворот камеры (с учётом поворота родителя,
//      который игра крутит сама через LookRoot: мы доворачиваем СВОЮ часть,
//      её часть — поверх).
//
// Почему так надёжнее накопителя:
//   * нет объекта, который можно потерять — есть только камера, и она есть,
//     пока идёт игра;
//   * абсолютный замкнутый контур: ошибка всегда считается от ТЕКУЩЕЙ
//     позы, поэтому запись, которую игра перебила своим вводом,
//     компенсируется следующим же тактом (магнит на голову);
//   * нечему «разойтись с экраном»: нет второго места, где хранится угол.
//
// false + err_deg < 0 — слота камеры нет (не подтвердился по матрице вида);
// false + err_deg == 0 — цель в самой камере (стрелять некуда);
// true — запись сделана (или цель уже в прицеле — тогда applied_deg == 0).
//   err_deg    — угол между осью камеры и целью, градусы;
//   applied_deg — на сколько градусов довернули в этом такте.

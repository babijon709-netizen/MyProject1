...
// Это тот самый множитель, которым игра превращает накопленный сдвиг касания в
// поворот камеры, поэтому град/px пропорционален ему: аим обязан брать
// коэффициент отсюда, а не из зашитого числа, иначе при своей чувствительности
// игрока он либо не доводит цель, либо перелетает её.
// false — прочитать не удалось (нет привязки, нет локального игрока/мыши):
// вызывающий берёт запасное значение.
...
// ---- Мемори-аим: прицел ведётся записью в память игры ----------------------
//
// Тач-аим крутит камеру синтетическим пальцем (uinput). Мемори-аим пишет прямо
// в память игры, поэтому не зависит от инъекции касаний: работает там, где
// /dev/uinput занят, перехвачен или его нет вовсе.
//
// Оба режима пишут в тот объект, которым игра сама вертит камерой
// (Oxide.MouseLook; PlayerManager.mouseLook, +0x70):
//
//   * «память»  — пишем ввод взгляда в то же поле, куда игра кладёт сдвиг
//     касания за кадр (+0x88, Vector2). Поворот дальше делает САМА игра своим
//     Update: теми же пределами углов, тем же KCC, той же отдачей. Камера
//     едет ровно как от настоящего свайпа — только без пальца.
//
//   * «сайлент» — пишем ось выстрела (PlayerEventHandler.LookDirection):
//     вдоль неё FPHitscan пускает луч попадания. Камера стоит на месте, в цель
//     уходит только выстрел.
//
// Единицы ввода MouseLook: игра умножает значение поля на m_Sensitivity и
// получает ГРАДУСЫ. Видно в MouseLook.ZJo (libil2cpp.so, RVA 0x64e312c):
// прочитанное по +0x88 множится на m_Sensitivity (+0x34) и складывается с
// углами взгляда (+0x4C тангаж, +0x50 рысканье), которые тут же зажимаются
// пределами m_DefaultLookLimits (+0x38, Vector2 min/max) и оборачиваются по
// 360. Значит «градус на единицу ввода» = m_Sensitivity ровно, и переводить
// градусы в единицы аиму больше нечем.

// Параметры ввода взгляда у локального игрока:
//   deg_per_unit — сколько градусов даёт одна единица поля ввода (= m_Sensitivity);
//   invert_y     — инверсия вертикали (m_Invert): знак вертикальной оси другой;
//   gyro         — гироскоп включён: игру НЕ обнуляет поле ввода, а ПРИБАВЛЯЕТ
//                  к нему своё, поэтому записанное может сложиться с её вводом.
// false — объект в этом кадре недоступен (нет привязки, нет локального игрока).
bool esp_mem_aim_look_params(float& deg_per_unit, bool& invert_y, bool& gyro);
// Накопитель углов камеры (MouseLook, +0x4C/+0x50) — единственное место, куда
// имеет смысл писать поворот. Почему не поле ввода взгляда (+0x88): ZJo
// сначала зовёт ZJX, а тот перезаписывает +0x88 вводом касания
// (str d0, [x19, #0x88], файл libil2cpp.so 0x64e0364), и только потом ZJo
// читает это поле. Запись между кадрами игры стирается гарантированно.
// С накопителем иначе: ZJo делает angles = clamp(angles + input * sens)
// (ldur d1,[x19,#0x4c] -> fadd -> stur d0,[x19,#0x4c], файл 0x64df2b8), то
// есть наше значение — база, к которой игра прибавляет свой ввод (ноль, если
// экран не трогают) и клампит пределом обзора.
//
// Единицы — градусы, и это же убирает всю возню с коэффициентом: никакой
// m_Sensitivity для перевода не нужен.
//   x_deg — rotationX камеры: в Unity это «наклон вниз», то есть
//           ОТРИЦАТЕЛЬНОЕ возвышение (смотреть вверх = уменьшать);
//   y_deg — rotationY: рысканье, вправо — больше.
...
// нужно ТО, что там лежит сейчас, а не абсолютный угол.
...
// Держать ось выстрела фоновым писателем (период 12 мс — игра кладёт ось раз
// в кадр, 30-60 Гц, и этого хватает). Игра кладёт свою ось каждый кадр,
// поэтому одна запись за кадр оверлея оказывается перезаписанной до выстрела:
// писатель-доминатор добивает ось тем же направлением между кадрами.
//
// Период взят не «как можно чаще»: каждая запись в /proc/<pid>/mem берёт
// mmap_lock процесса игры, и на 2 мс (500 записей/с) игра вставала совсем —
// вместе с оверлеем, который читает память через тот же файл. 12 мс (~83
// записи/с) кадр игры всё ещё перекрывает, а нагрузки почти нет.
//
// Адрес и направление даёт поток отрисовки (esp_mem_aim_write_fire_dir):
// сам писатель ничего не разресолвит, чтобы не трогать общий кэш игры из
// второго потока. Пока включён, направление обновляется каждым вызовом
// esp_mem_aim_write_fire_dir().

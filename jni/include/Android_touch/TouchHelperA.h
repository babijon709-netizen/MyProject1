...
// Можно ли инжектить касания в принципе: проба /dev/uinput без захвата
// тачскрина и без пересоздания чего-либо (см. TouchHelperA.cpp). Поток
// переподключения спрашивает это ПЕРЕД полным Touch_Init.
bool Touch_ProbeInject();

// Диагностика: сколько раз Upload() собрал пакет синтетического тача, сколько
// миллисекунд на это ушло суммарно и сколько вызовов обошлось без отправки
// (пакет был тот же, что уже ушёл в uinput). Счётчики накопительные, хозяин

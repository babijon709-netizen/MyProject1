...
// ---- Проверка FT-драйвера ---------------------------------------------------
// true — FT-драйвер прошит в ядро и патч активен: непривилегированный
// процесс может читать чужую память (драйвер снимает запреты на уровне
// ядра). false — драйвера нет: KERNEL-режим запускать нельзя.
bool        driver_active();

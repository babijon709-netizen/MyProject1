...
void UpdateAim(float dt) {
    ...
    // Признак «аим ведёт камеру» — на этот такт. Ниже он поднимается только там,
    // где цель действительно взята (см. AimIsDriving): автофарм смотрит на него,
    // чтобы не тянуть камеру одновременно с аимом.
    AimSetMemoryDriving(false);
    ...
    //
    // После отказа устройства (MEM_AIM_UNSUPPORTED) самотест молчит: палец в
    // этом режиме не используется, камеру никто не трогает, и мерить нечего.
    // Перепроверка — на смене режима, на перепривязке (esp_reset ->
    // esp_mem_aim_reset) и на смене поколения мира (см. ниже).
    int memState = esp_mem_aim_state();
    if (active && memoryMode && memState != MEM_AIM_UNSUPPORTED) esp_mem_aim_tick(dt);
    if (active && memoryMode && memState == MEM_AIM_UNSUPPORTED) {
        // Отказ («ни одна дорожка не подтвердилась») не приговор на весь сеанс:
        // самотест мог провалиться в неудачный момент — например, пока после
        // смерти грузился мир, и адреса ещё не были готовы. Пробуем заново, но
        // только когда мир действительно сменился (новое поколение) или чит был
        // перепривязан: иначе на устройстве, где записи нет вовсе, мы бы каждые
        // несколько секунд дёргали прицел пробными доворотами.
        static int s_memFailedGeneration = -2;   // -2: отказа ещё не видели
        if (s_memFailedGeneration == -2) {
    ...
    // ПРАВИЛО: в режиме «Мемори» палец не участвует ни в одной ветке — ни при
    // готовой дорожке, ни во время самотеста, ни при отказе (см. ниже).
    if (memoryMode) {
        if (memState == MEM_AIM_READY) {
            AimReleaseFinger(s_fingerDown);
            AimSetMemoryDriving(true);       // камера занята: автофарм уступает её
        ...
        // Дорожки записи нет. РАНЬШЕ здесь аим «на всякий случай» уходил в
        // тач-ветку ниже — и режим «Мемори» водил палец, чего в нём быть не
        // должно вовсе (палец — это видимый тач по экрану, квант цифровера и
        // чужая чувствительность). Теперь отказ так и остаётся отказом: палец
        // отпущен, камеру не трогаем, а причина написана во вкладке «Аим».
        AimReleaseFinger(s_fingerDown);
        s_haveLast = false;
        return;
    }

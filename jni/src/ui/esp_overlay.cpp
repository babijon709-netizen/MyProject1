...
// что здесь лежит и кто это зовёт — в шапке ui/esp_overlay.h и в docs/CODE_MAP.md.
...
// 1.5 с, а не 0.7 с: на слабом устройстве чтение списка игроков и позиций идёт
// рывками, и «пустых» кадров подряд бывает больше, чем один — рамки успевали
// погаснуть и вернуться, это и виделось как мерцание ESP.
//
...
// новый: 1.5 с — это ещё не «игрок ушёл», зато переживает перезагрузку мира.
static constexpr double kBoxHoldSeconds = 1.5;
...
// kPosHoldReloadSeconds в player_pose.cpp), чтобы слои не разъезжались.
static constexpr double kBoxHoldStaleSeconds = 6.0;

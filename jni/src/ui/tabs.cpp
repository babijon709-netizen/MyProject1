...
float TabContent(int tab, float dt, float cW) {
    ...
    if (tab == 1) {
        ...
        // трогает — поэтому строка обязана быть видна. ВАЖНО: отказ режима «Мемори»
        // больше не подменяется тач-веткой — палец в этом режиме не используется
        // вообще (см. aim/update.cpp), поэтому строка прямо говорит, что аим не
        // работает, и называет причину.
        SHdr(XS("Режим"));
        {
        ...
        if (g_state.aim_mode == 1) {
            ...
            if (!g_state.aim_touch) {
                ...
                // Палец здесь не используется намеренно: «Мемори» — это режим без
                // тача, и «на всякий случай» водить палец нельзя. Поэтому строка
                // честно говорит, что аим молчит, и называет причину.
                head = XS("Через память нельзя — аим не работает");
                headCol = C::Red();

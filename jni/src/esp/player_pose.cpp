...
// «ESP мерцает». 2.0 с — это ещё не «игрок вышел», зато переживает перезагрузку
// мира; во время самой перезагрузки держим ещё дольше (там данные заведомо
// недописаны, и гаснуть раньше игры просто не за чем). Окна согласованы с
// удержанием снимка в рисующем слое (esp_overlay.cpp), чтобы слои не разъезжались.
static constexpr double kPosHoldSeconds       = 2.0;

static constexpr double kPosHoldReloadSeconds = 6.0;

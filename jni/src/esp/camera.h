...
// Ось прицела взята без сверки с позой камеры: сверять было не с чем, потому что
// поза в этот момент не читается. Ось при этом настоящая — она берётся из
// обработчика событий игрока (LookDirection), а не из камеры.
...
// Расхождение позы камеры с углами прицела игры (градусы; -1 — замерить не
// вышло). Замер для журнала: по нему видно, можно ли доверять осям.
extern float g_cam_view_angle_gap_deg;
// Откуда взят поворот вида (только для журнала и только у ESP): 0 — поза
// камеры, 1 — ось прицела (поза камеры не читается), 2 — удержанный/кэш камеры.
// g_cam_forward/g_cam_pose_valid НЕ переписываются: на них стоит тач-аим, и
// подмена оси видом уже один раз сломала наведение (17 сентября 2026).
extern int   g_cam_view_source;
// Поза камеры признана негодной по расхождению с осью прицела и углами прицела
// игры (журнал устройства 17.09.2026: «все игроки ЗА камерой»). Вид ESP тогда
// собирается по оси прицела.
extern bool  g_cam_pose_bad;
// Кадр за кадром ВСЕ игроки оказываются за камерой: вид устарел (после смерти
// камера уезжает, а позу с устройства прочитать не удаётся — мы держим прошлый
// вид). По этому флагу вид на следующем кадре собирается по оси прицела.
extern bool  g_cam_view_suspect;

...
// Это матрица ВИДА (worldToCamera)? 0 — не похожа, 1 — похожа как есть, 2 — похожа
// после перестановки (нативная раскладка построчная, см. mat_transposed выше).
//
// Зачем отдельная проверка. Матрицу вида мы обычно собираем сами из позы камеры,
// а из памяти игры читаем только резервом. И вот этот резерв был источником
// «все игроки ЗА камерой» в журнале устройства 17.09.2026: нативная матрица
// читалась «как есть», то есть перевёрнутой, у неё ВЫВЕРНУТ поворот — а перевод
// (последний столбец против последней строки) читается одинаково, поэтому
// позиция камеры выглядела правильной, и ошибку не было видно. Точки при этом
// уходили за камеру, и все рамки отбрасывались как «ниже экрана».
//
// Проверка структурная: поворотная часть ортонормированна (строки единичной
// длины — для раскладки «как есть», столбцы — для переставленной), последняя
// строка перевода нулевая, (3,3) == 1. Случайные 64 байта этому не отвечают.

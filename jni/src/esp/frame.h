...
// Был ли хоть один собранный кадр с момента привязки к процессу. Нужно сторожу:
// пока кадра не было НИ ОДНОГО, чит, скорее всего, просто ждёт, пока игра
// загрузит мир (указателя камеры в памяти ещё нет) — сбрасывать кэши и
// перепривязываться в этом состоянии бессмысленно и вредно. А вот если кадры
// шли и вдруг пропали — это уже поломка, и вести себя надо как раньше.
extern bool g_frame_ever_published;

// Сколько кадров опубликовано (растёт в frame_note_published). По нему видно,
// «мир только что стал читаемым» — на этот переход завязан повтор самотеста
// мемори-аима (см. aim/update.cpp).
extern unsigned long long g_frame_publish_count;

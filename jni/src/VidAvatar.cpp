...
// tools/video/gen_avatar.py из mp4). Формат кадра — обычный baseline JPEG,
...
// ~0.2 мс на кадр 96x96 (замер на x86; на телефоне ожидаемо в разы больше, но
// всё равно доли миллисекунды на кадр экрана). Один кадр ролика декодируется
...
void upload() {
    if (!g_tex) {
        ...
        // Ширина кадра 96x3 = 288 байт — кратно 4, поэтому выравнивание строк

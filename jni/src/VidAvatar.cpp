...
// tools/video/gen_avatar.py из mp4; сейчас 256x256, 12 кадров/с — кружок в
// меню 176 px, то есть кадр показывается в натуральную величину). Формат
...
// ~0.4 мс на кадр 256x256 (замер на x86; на телефоне ожидаемо в разы больше,
...
void upload() {
    if (!g_tex) {
        ...
        // Ширина кадра 256x3 = 768 байт — кратно 4, поэтому выравнивание строк

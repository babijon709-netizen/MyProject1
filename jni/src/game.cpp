...
// Второй ограничитель прохода — ВРЕМЕННОЙ, и на устройстве главный.
//
// «200 условных чтений» оценивают объём прохода, но не его цену: классификация
// новой записи — это не 12 syscall'ов, а три-четыре десятка (класс, список
// компонентов, каждый компонент, трансформ, имя префаба, loot-списки), а на
// придушенном телефоне каждый syscall стоит не 20 мкс, а все 100-200. Замер с
// устройства 15.09.2026 (farm_debug.log, разбор кадра): стадия farm держала
// 90.4..122.4 мс на кадр, сон до темпа 0.0, оверлей шёл 9-10 к/с вместо 60 —
// при том что esp был 0.5-2.6 мс, aim 0.0, menu 0.1-0.3. То есть кадр съедал
// именно скан реестра, и вместе с кадром тонул аим: он живёт в том же цикле и
// получал 9-10 обновлений в секунду вместо 60 (с выключенным фармом тот же
// кадр стоил 16.6 мс и оверлей шёл 59.1 к/с).
//
// Поэтому порция скана ограничена сверху ещё и миллисекундами: единицы держат
// объём прохода, время держит кадр. Лимит 6 мс при бюджете кадра 16.7 мс (60 Гц)
// оставляет место ESP, контроллеру, меню и ImGui/GL — ровно так же ограничен
// скан маркеров (kMarkerScanBudgetSec).
constexpr double kFarmScanBudgetSec = 0.006;
...
static void farm_scan_tick() {
    ...
    g_farm_scan_units = 0;
    g_farm_scan_new = 0;
    const double t_portion = mono_seconds();
    while (g_farm_scan_idx < g_farm_scan_ids.size()) {
        // Проверка до чтения записи: курсор ещё не сдвинут, поэтому прерывание
        // ничего не теряет — запись обработает следующий кадр.
        if (mono_seconds() - t_portion > kFarmScanBudgetSec) break;

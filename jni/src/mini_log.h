...
// никакой телеметрии по кадрам. Файл обрезается на 256 КБ с одним архивом .1,
// поэтому место не съест. Путь выбирает main.cpp (benzware.log в «Загрузках», а
// если те недоступны — рядом с конфигами).
//
...
// Фактический путь файла — для первой строки лога (в какой каталог попали).
inline const char* path() { return detail::path(); }
